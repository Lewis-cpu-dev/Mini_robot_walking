...
class MINI_Approx_ZMP_Single_Mass: 
    ...
    def calc_single_mass_zmp(self, x_pos, y_pos, z_pos, x_accl, y_accl, z_accl, g = 9.81, pz = 0.30):
        ...
        denominator = z_accl + g
        if denominator == 0:
            rospy.logwarn("Denominator in ZMP calculation is zero.")
            return x_pos, y_pos  # fallback to CoM position
        ...
        px = x_pos - ((z_pos - pz) * x_accl) / denominator
        py = y_pos - ((z_pos - pz) * y_accl) / denominator
